...
console.log(CurrentData)
  ...
  return (
    ...
    <Carousel  width={"100%"}  dynamicHeight={false} className={styles.carouselSetting? "showIndicators":""}  showIndicators={false} showStatus={false} showArrows={false} >
       ...
       <div style={{width:"100%",height:"600px",justifyContent:"center"}}>
       <img  width={"100%"} src={data.image4} alt="" />       </div>
       <div style={{width:"100%",height:"600px",justifyContent:"center"}}>
       <img   src={data.image5} alt="" />       </div>
    </Carousel>
    </Box>
    <Box>
      <br />
        ...
        <Box marginBottom={"50px"}><Text>{data.description}</Text></Box>
        {/* icon and head */}
      <Box>
      <Text></Text>
      <Text></Text>
      ...
      <Box marginBottom={"50px"} display={"flex"} justifyContent={'space-between'}>
        <Text textDecoration={"HighlightText"} fontSize={"2xl"}>${data.price}</Text>
        <Text>{data.percent}%</Text>

      </Box>
      <Box>
        <Progress value={data.percent} borderRadius={"5px"}
    ...
    <br/><br/>
      ...
      <Flex justifyContent={"space-between"}>
        <Button border={"1px solid grey"}
         ...
         borderRadius={"none"}   
         leftIcon={<BiHeart size={"25px"}  className={Heart===true? `${styles.heart}`:``}/>}>FOLLOW</Button>
         <Flex  gap="10px">
            <BsFacebook size={"20px"}/>
             <BsTwitter size={"20px"}/>
             <Image src="https://th.bing.com/th/id/OIP.ASjs8TW-8mI6rryXhKY0RwAAAA?pid=ImgDet&rs=1" width={"20px"} height="20px"/>

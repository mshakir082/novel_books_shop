...
const CartPage = () => {
  ...
  const cart_data = useSelector((state)=>state.appReducer.cart)
  var bag =  0 ; 
...
const handleSelect = (id,qyt)=>{
  ...
  var increase = {
    ...
    axios.patch(`https://novel-book.herokuapp.com/api/cart/${id}`,increase)
    .then((res)=>{
    ...
    <Box
     ...
     >
      ...
      <Table >
        ...
        <Box textAlign={{md:'right',base:'center'}} padding={{md:'0.5rem',base:'0.8rem'}} >
        <Button bg='#363F43' color='white' mr='5px'>Continue Shopping</Button>
        <Button bg='#363F43' color='white' mr='5px' >Update Cart</Button>
        <Button bg='#363F43' color='white' mt={{base:'5px',md:'-2px'}} onClick={handelClick}>Checkout</Button>
        </Box>
   </Box>
   </div>
    </>
